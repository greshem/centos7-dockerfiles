...
#build dockerfile
#docker build  --no-cache  -t greshem/collectd:centos7 .
docker build    -t greshem/collectd:centos7 .

echo  docker run -it     greshem/collectd:centos7 .
echo  docker run -d      greshem/collectd:centos7 

#get image id
#IMAGE_ID=`docker images | grep greshem/collectd | awk '{print $3}'`
